...
class DataStore
{
    ...
    protected function get_status_subquery($query_args, $operator = 'AND')
    {
        ...
        if (isset($query_args['status_is']) && is_array($query_args['status_is']) && count($query_args['status_is']) > 0) {
            ...
            if ($allowed_statuses) {
                $subqueries[] = "{$wpdb->prefix}qa_cog_order_stats.status IN ( '" . implode("','", $allowed_statuses) . "' )";
            }
        }
        ...
        if ($excluded_statuses) {
            $subqueries[] = "{$wpdb->prefix}qa_cog_order_stats.status NOT IN ( '" . implode("','", $excluded_statuses) . "' )";
        }
