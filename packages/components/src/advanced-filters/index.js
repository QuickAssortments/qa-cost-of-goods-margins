...
const matches = [
	{ value: 'all', label: __( 'All', 'qa-cost-of-goods-margins' ) },
	{ value: 'any', label: __( 'Any', 'qa-cost-of-goods-margins' ) },
];
			...
			components: {
				select: (
					<SelectControl
						...
						aria-label={ __( 'Choose to apply any or all filters', 'qa-cost-of-goods-margins' ) }
					/>
				),
			},
		} );
					...
					<div className="woocommerce-filters-advanced__add-filter">
						<Dropdown
							...
							renderToggle={ ( { isOpen, onToggle } ) => (
								...
								>
									{ __( 'Add a Filter', 'qa-cost-of-goods-margins' ) }
								</IconButton>
							) }
				...
				<div className="woocommerce-filters-advanced__controls">
					{ updateDisabled && (
						<Button isPrimary disabled>
							{ __( 'Filter', 'qa-cost-of-goods-margins' ) }
						</Button>
					) }
					{ ! updateDisabled && (
						<Link
					...
					{ activeFilters.length > 0 && (
						<Link type="wc-admin" href={ this.getUpdateHref( [] ) } onClick={ this.clearFilters }>
							{ __( 'Clear all filters', 'qa-cost-of-goods-margins' ) }
						</Link>
					) }
				</div>
			</Card>
