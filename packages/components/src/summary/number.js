...
const SummaryNumber = ( {
	...
	let screenReaderLabel =
		delta > 0
			? sprintf( __( 'Up %d%% from %s', 'qa-cost-of-goods-margins' ), delta, prevLabel )
			: sprintf( __( 'Down %d%% from %s', 'qa-cost-of-goods-margins' ), Math.abs( delta ), prevLabel );
	if ( ! delta ) {
		// delta is zero or falsey
		icon = 'arrow-right';
		screenReaderLabel = sprintf( __( 'No change from %s', 'qa-cost-of-goods-margins' ), prevLabel );
	}
			...
			<Container { ...containerProps }>
				...
				<span className="woocommerce-summary__item-data">
					<span className="woocommerce-summary__item-value">
						{ ! isNil( value ) ? value : __( 'N/A', 'qa-cost-of-goods-margins' ) }
					</span>
					<div
					...
					>
						...
						<span className="woocommerce-summary__item-delta-value">
							{ ! isNil( delta )
								? sprintf( __( '%d%%', 'qa-cost-of-goods-margins' ), delta )
								: __( 'N/A', 'qa-cost-of-goods-margins' ) }
						</span>
					</div>
				</span>
				...
				<span className="woocommerce-summary__item-prev-value">
					{ ! isNil( prevValue ) ? prevValue : __( 'N/A', 'qa-cost-of-goods-margins' ) }
				</span>
...
SummaryNumber.defaultProps = {
	href: '',
	isOpen: false,
	prevLabel: __( 'Previous Period:', 'qa-cost-of-goods-margins' ),
	reverseTrend: false,
	selected: false,
