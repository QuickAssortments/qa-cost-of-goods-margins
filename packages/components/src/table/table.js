		...
		return (
			...
			>
				<table>
					...
					>
						{ caption }
						{ tabIndex === '0' && <small>{ __( '(scroll to see more)', 'qa-cost-of-goods-margins' ) }</small> }
					</caption>
					<tbody>
						<tr>
							{ headers.map( ( header, i ) => {
								...
								const iconLabel =
									sortedBy === key && sortDir !== ASC
										? sprintf( __( 'Sort by %s in ascending order', 'qa-cost-of-goods-margins' ), screenReaderLabel )
										: sprintf( __( 'Sort by %s in descending order', 'qa-cost-of-goods-margins' ), screenReaderLabel );
							...
							: (
								<tr>
									<td className="woocommerce-table__empty-item" colSpan={ headers.length }>
										{ __( 'No data to display', 'qa-cost-of-goods-margins' ) }
									</td>
								</tr>
							)
						}
