			...
			<Fragment>
				{ selected.map( ( item, i ) => {
					...
					const screenReaderLabel = sprintf(
						__( '%1$s (%2$s of %3$s)', 'qa-cost-of-goods-margins' ),
						item.label,
						i + 1,
						selected.length
					);
		...
		return (
			<div className={ classnames( 'woocommerce-search', className, {
							...
							>
								...
								<div className="woocommerce-search__token-list">
									...
									<span id={ `search-inline-input-${ instanceId }` } className="screen-reader-text">
										{ __( 'Move backward for selected items', 'qa-cost-of-goods-margins' ) }
									</span>
								</div>
							</div>
						) : (
				...
				{ showClearButton && shouldRenderTags ? (
					...
					>
						<Icon icon="dismiss" />
						<span className="screen-reader-text">{ __( 'Clear all', 'qa-cost-of-goods-margins' ) }</span>
					</Button>
				) : null }
			</div>
		);
