...
export default {
	...
	options( search ) {
		...
		return apiFetch( { path: addQueryArgs( '/qa/v1/products/categories', query ) } );
	},
	isDebounced: true,
		...
		const label = (
			<span key="name" className="woocommerce-search__result-name">
				{ interpolateComponents( {
					mixedString: __( 'All categories with titles that include {{query /}}', 'qa-cost-of-goods-margins' ),
					components: {
